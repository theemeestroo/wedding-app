...
Phases 0 (Foundation), 1 (Venue Board), and 2 (Enquiry CRM) are built and verified end-to-end against the live Supabase project — see [implementation-plan.md](implementation-plan.md) §0 and §4 for exactly what that covers. Auth, locale routing, projects/membership/invites, the venue board, and the enquiry pipeline (status tracking, timeline, quotes, follow-ups, needs-attention dashboard widget, draft-message generator) all work. No design system yet — plain neutral shadcn theme throughout, no map/cost-engine/logistics work has started (that's Phases 4–5).
