...
`wedding.households` (tier A–D and `group_label` live here, not on `guests` — see AGENTS.md's note on why), `wedding.guests` (name, child flag, and `rsvp_status` — see AGENTS.md's note on why that's guest-level and informational-only), `wedding.origin_clusters` + `wedding.recompute_origin_clusters()` (2° grid bucketing, only ever touches unassigned households — see AGENTS.md), `wedding.guest_plans` + `wedding.guest_plan_exceptions` (tier/group rules with per-household overrides; headcount computed client-side from already-fetched data, never stored). Geocoding via Nominatim (`app/api/geocode/route.ts`) backed by the global `wedding.geocode_cache`. CSV import (`/guests/import`) — hand-rolled RFC4180 parser, column mapping, fuzzy household grouping by normalized surname+city, sequential geocoding respecting Nominatim's rate limit.

The Guests page (`app/[lang]/(app)/guests/page.tsx`) was later restructured from one flat scroll into tabs (`components/guests/guests-tabs.tsx`, mirroring `option-detail.tsx`'s tab pattern): **Overview** (stat cards + by-tier/by-group/by-RSVP-status breakdowns, click-through to a filtered Guest List), **Households** (the original add-form + household-card list, now filterable and grouped by tier/group by default), **Guest List** (a flat per-person table — every guest across every household, with inline-editable RSVP status), and **Origins** (unchanged `OriginClusterPanel`). Household naming was also fixed: `households.name` is now an explicit, always-visible, always-editable field (defaulting to the actual guest's name, e.g. "John Smith", not the collision-prone "Smith household" every same-surname household used to get silently), with a non-blocking duplicate-name warning in `household-card.tsx` when two households in the same project share a name.
...
### Phase 7b — Individual rooms, room-level allocation ✅ built
Reworked Phase 7's accommodation model from a flat per-property record (`rooms_available`/`capacity_adults`/`capacity_children`/`nightly_rate` all on one `accommodations` row) into a real two-level model: `wedding.accommodations` stays the property record (name/type/confidence), and a new `wedding.accommodation_rooms` table holds the actual bookable unit — capacity, room type and nightly rate per room, since a single hotel/villa can have several room types. The Venue → Accommodation tab (`components/venues/venue-accommodation-tab.tsx` + new `accommodation-rooms-manager.tsx`) gained a "block create" flow: one form inserts N room rows sharing type/capacity/rate with auto-numbered labels — no `quantity` column, every room is its own row from creation.
`wedding.allocations` was reworked to point at a specific `room_id` rather than an `accommodation_id` + bare `rooms_assigned` count — a household needing more than one room now just gets more than one allocation row. `unique(option_id, room_id)` (replacing the old `unique(option_id, household_id)`) is the DB-level guarantee that a room can't be double-booked within an Option; the option detail page's room pickers only ever offer unbooked rooms as the UI-level backstop. `lib/accommodation-engine.ts` gained `computeRoomOccupancy()` (pure lookup, no persistence) backing a "N/M rooms booked" badge per property on the option's Accommodation tab, and `computeAccommodationCost()`/`AllocationInput` were reshaped to room-level (`roomId` instead of `accommodationId` + `roomsAssigned`).
**Also**: `components/options/option-detail.tsx` (1029 lines, one long scroll) was split into a tabbed shell + five sibling tab components (`option-logistics-tab.tsx`, `option-accommodation-tab.tsx`, `option-events-tab.tsx`, `option-costs-tab.tsx`, `option-decision-tab.tsx`), mirroring `venue-detail-tabs.tsx`'s existing tab pattern — only the cost summary hero and the decided/not-decided banners stay outside the tabs, since they're page-level status, not one section's content. Shared row types/formatters extracted to `components/options/shared.ts` since five sibling files now need them.
**Migration note**: this was a clean drop-and-recreate of `accommodations`'/`allocations`' old columns/shape, not a data-preserving transform — existing rows predated any real couple using the app (see AGENTS.md's testing-convention note), per explicit user decision.
**Exit — met**: a couple can block-create a batch of rooms for a property in one form, see per-property "N/M rooms booked" occupancy at a glance on an Option's Accommodation tab, assign a household to more than one room, and get a non-blocking capacity warning when a household's headcount exceeds its assigned room's capacity.

### Phase 7c — Block-priced accommodation ✅ built
Phase 7b's room-level model (capacity/type/rate all on `accommodation_rooms`) can't represent a venue that quotes exclusive use of the whole property as one number, not per room — confirmed against a real venue proposal entered into a real couple's project on 2026-08-20 (Ca' Salva: "EUR2,000/night, exclusive use of all 21 beds", never priced per apartment; contrast with the same project's Borgo Pianello, whose 8 differently-configured apartments genuinely are per-room-capable). `wedding.accommodations` gained a `pricing_mode` switch (`'per_room'`, the default — cost stays on each room's `nightly_rate`, Phase 7b's behaviour unchanged; or `'block'` — cost is a single `block_nightly_rate`/`block_currency` on the accommodation itself, enforced non-null by a check constraint when that mode is selected). `wedding.accommodation_rooms.nightly_rate` became nullable — under a block-priced accommodation, rooms exist only to label which named unit a household is allocated to, with no cost meaning of their own.
`lib/accommodation-engine.ts`'s `computeAccommodationCost()` gained an `accommodations: AccommodationInput[]` parameter and now groups allocations by their room's parent accommodation before pricing: per-room allocations are priced exactly as before (one row per allocation), block-mode allocations are priced once per accommodation regardless of how many households/rooms are allocated to it, over the span from the earliest arrival to the latest departure among everyone allocated to it (same 2-night default as the per-room path when no arrival profile is set). The existing Phase 7 "Estimate accommodation & transfer costs" button and its `cost_rules` upsert mechanism needed no changes — it already just forwards whatever total the engine produces.
**Verified live**, not just type-checked: real project ("Richard and Karlottas Wedding"), real login (the app's `/auth/callback` is PKCE-only and doesn't handle `supabase.auth.admin.generateLink()`'s implicit-flow output, so verification completed the magiclink token server-side via a direct REST call and set the `@supabase/ssr` session cookie by hand rather than fighting the mismatch). Added a block-priced accommodation through the Venue → Accommodation tab, added a room with a blank nightly rate (confirmed not required in block mode, with an explanatory note shown), allocated a household to it from the Option's Accommodation tab, and clicked "Estimate costs" — produced "Accommodation (auto-calculated): EUR4,000" (EUR2,000 x the 2-night default), zero browser console errors throughout. Test data (the accommodation, its room, the allocation, and the auto-generated cost_rules row) was deleted afterward rather than left in the couple's real project.
**Exit — met**: a couple can represent either a venue that prices per room (existing behaviour, unchanged) or one that prices exclusive use of the whole property as a single nightly rate, and either way that cost flows into the same estimate-and-apply mechanism as before.
