...
### Phase 2 — Enquiry CRM
`wedding.enquiries` (status pipeline: not contacted → drafting → sent → awaiting response → replied → follow-up required → quote received → viewing booked → shortlisted / no availability / rejected / booked), `wedding.enquiry_events` (timeline), `wedding.quotes`, document upload. Inbound-parse webhook + per-project unique inbound address (`proj-a1b2@in.yourdomain.com`) — this is the one piece of Phase 2 that's infra, not just schema+UI; needs an email-receiving domain configured before this phase can finish.
**Exit**: dashboard can honestly say "3 venues awaiting reply, 2 follow-ups due this week."
